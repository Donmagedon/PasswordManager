...
function formattedTime(){
        ...
        let current = `${dateInstance.getFullYear()}/${dateInstance.getMonth() + 1 }/${dateInstance.getDate()}/${dateInstance.getHours()}:${dateInstance.getMinutes()}`
        return current
...
async function loginAttempt(req,res,next){
    const savedPassword = await users.findOne(
        {username:req.body.username}
    )
    try{

        const validation = bcrypt.compare(req.body.password,savedPassword.password)
        if(await validation){
            next()
        }else{
...
async function tokenCreation(req,res,next,){
    ...
    const tokenLate = JWT.sign({username:req.body.username},
        key,{algorithm:"RS256",expiresIn:"3d"})
    const tokenEarly = JWT.sign({
        username:req.body.username,
        password:req.body.password,
    },key,{algorithm:"RS256",expiresIn:"3h"})
    res.json({tokenEarly,tokenLate})
}

async function sessionSaved(req,res){
    const key = fs.readFileSync(path.join(__dirname,"../../certificates/public.key"))
...
async function createPassword(req,res,next){
    ...
    const encrypt = function(password){
        const public = fs.readFileSync(path.join(__dirname,"../../certificates/public.key"))
    ...
    try{
        const titleExists = await passwordObject.findOne({title:passwordData.title})
        if(titleExists){
            res.sendStatus(400)
        }else{
...
async function decryptPassword(input){
    const private = fs.readFileSync(path.join(__dirname,"../../certificates/private.key"))
    const decrypted = crypto.privateDecrypt(private,Buffer.from(input,"base64"))
